...
from logistic_sgd_global_onesided2_human import LogisticRegression, load_input_data, load_output_data
...
def evaluate_cnn(dataset='bio_larger'):#_unfiltered
	...
	fit_apadb(cnn, test_set_x_prox_aligned, test_set_x_dist_aligned, test_set_d_prox, test_set_d_dist, test_set_y, test_set_L, test_set_d, test_set_c, test_set_s, dataset)'''
	

	fit_apadb(cnn, test_set_x_prox, test_set_x_dist, test_set_d_prox, test_set_d_dist, test_set_y, test_set_L, test_set_d, test_set_c, test_set_s, dataset)

def fit_apadb(cnn, test_set_x_prox, test_set_x_dist, test_set_d_prox, test_set_d_dist, test_set_y, test_set_L, test_set_d, test_set_c, test_set_s, dataset) :
	...
	plt.title('APADB Testset (R^2 = ' + str(round(lr.score(X_test, p_test), 2)) + ', Acc = ' + str(round(accuracy, 2)) + ')' + ', ' + str(X_test.shape[0]) + ' UTRs', fontsize=18)

	#plt.subplots_adjust(top=0.85)

	plt.savefig("onesided2_dropout_cnn_apadb_testset_" + dataset + ".png")
	plt.savefig("onesided2_dropout_cnn_apadb_testset_" + dataset + ".svg")
	plt.show()
	...
	r_packet_native_lr = numpy.load("native_lr_apadb_testset_roc_" + dataset + ".npy")
	...
	r_packet_native_cnn = numpy.load("native_cnn_apadb_testset_roc_" + dataset + ".npy")
	...
	l1, = plt.plot(fpr, tpr, c = 'orange', linewidth=4, alpha=0.7)
	...
	plt.title('APADB Test ROC' + ', ' + str(X_test.shape[0]) + ' UTRs', fontsize=18)

	plt.savefig("combined_onesided2_dropout_cnn_apadb_testset_roc_" + dataset + ".png")
	plt.savefig("combined_onesided2_dropout_cnn_apadb_testset_roc_" + dataset + ".svg")
	plt.show()
	...
	plt.xlabel('Predicted Proximal Logodds', fontsize=18)
	plt.ylabel('Observed Proximal Logodds', fontsize=18)
	plt.title('APADB LOOCV (R^2 = ' + str(round(test_rsquare, 2)) + ', Acc = ' + str(round(accuracy, 2)) + ')' + ', ' + str(X.shape[0]) + ' UTRs', fontsize=18)

	#plt.subplots_adjust(top=0.85)

	plt.savefig("onesided2_dropout_cnn_apadb_loocv_" + dataset + ".png")
	plt.savefig("onesided2_dropout_cnn_apadb_loocv_" + dataset + ".svg")
	plt.show()
	...
	r_packet_native_lr = numpy.load("native_lr_apadb_loocv_roc_" + dataset + ".npy")
	r_hat_native_lr = numpy.ravel(r_packet_native_lr[:, 0])
	r_native_lr = numpy.ravel(r_packet_native_lr[:, 1])
	...
	plt.title('APADB LOOCV ROC' + ', ' + str(X.shape[0]) + ' UTRs', fontsize=16)

	#plt.subplots_adjust(top=0.85)

	plt.savefig("combined_onesided2_dropout_cnn_apadb_loocv_" + dataset + ".png")
	plt.savefig("combined_onesided2_dropout_cnn_apadb_loocv_" + dataset + ".svg")
	plt.show()
