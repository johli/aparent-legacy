...
from logistic_sgd_global_onesided2_apadb import LogisticRegression, load_input_data, load_output_data
...
def align_on_max_score(cnn, ref_x, ref_d, L_zero) :
	...
	best_align_x = numpy.zeros((n, ref_x.shape[1], 4), dtype=theano.config.floatX)
	...
	for i in range(0, n) :
		...
		ref_x_curr = numpy.zeros((1, ref_x.shape[1], 4), dtype=theano.config.floatX)
		ref_x_curr[:, :, :] = ref_x[i, :, :]
		...
		if ref_seq[49:55] not in ['AATAAA', 'ATTAAA'] :
			for j in range(align_min, align_max) :
				...
				y_ref_hat = logit(cnn.get_online_prediction(align_x_curr.astype(theano.config.floatX), L_zero_curr.astype(theano.config.floatX), ref_d_curr.astype(theano.config.floatX)))[0]

				ref_seq_aligned = translate_matrix_to_seq(align_x_curr[0, :, :])

				if y_ref_hat > best_align_hat and ref_seq_aligned[49:55] in valid_pas : # and (j == 0 or numpy.abs(j) > 2)
...
def evaluate_cnn(dataset='apadb_v3'):#_unfiltered#apadb_v3

	count_filter=40#100#2000#1500#1000#800#400#100#40

	for count_filter in [1000] : #[40, 100, 200, 400, 500, 800, 1000, 1500, 2000] :
		...
		train_set_proxid = input_datasets_prox[17]
		valid_set_proxid = input_datasets_prox[18]
		test_set_proxid = input_datasets_prox[19]
		...
		run_name = '_Global2_Onesided2AntimisprimeOrigDropout_HumanAPADB_Six_30_31_34'
		...
		cnn = DualCNN(
			(train_set_x_prox, train_set_y, train_set_L, train_set_d_prox),
			(valid_set_x_prox, valid_set_y, valid_set_L, valid_set_d_prox),
			learning_rate=0.1,
			drop=0.2,
			n_epochs=10,
			...
			#store_as_dataset='general' + 'apa_sparse_general' + '_global_onesided2antimisprimeorigdropout_finetuned_TOMM5_APA_Six_30_31_34_pasaligned'
			...
			#wt_dropout
			#dataset='general' + 'apa_sparse_general' + '_global_onesided2antimisprimeorigwtdropout_finetuned_TOMM5_APA_Six_30_31_34_pasaligned_channeldep',#(_03) _05_1
			#store_as_dataset='general' + 'apa_sparse_general' + '_global_onesided2antimisprimeorigwtdropout_finetuned_TOMM5_APA_Six_30_31_34_pasaligned_channeldep'
			#dataset='general' + 'apa_sparse_general' + '_global_onesided2antimisprimeorigwtdropout_finetuned_TOMM5_APA_Six_30_31_34_pasaligned_channelindep',#(_03) _05
			#store_as_dataset='general' + 'apa_sparse_general' + '_global_onesided2antimisprimeorigwtdropout_finetuned_TOMM5_APA_Six_30_31_34_pasaligned_channelindep'
		...
		fit_apadb(cnn, test_set_x_prox_aligned, test_set_x_dist_aligned, test_set_d_prox, test_set_d_dist, test_set_y, test_set_L, test_set_d, test_set_c, test_set_s, dataset, count_filter)'''
		

		fit_apadb(cnn, test_set_x_prox, test_set_x_dist, test_set_d_prox, test_set_d_dist, test_set_y, test_set_L, test_set_d, test_set_c, test_set_s, dataset, count_filter, test_set_proxid, shuffle=True)

def fit_apadb(cnn, test_set_x_prox, test_set_x_dist, test_set_d_prox, test_set_d_dist, test_set_y, test_set_L, test_set_d, test_set_c, test_set_s, dataset, count_filter, test_set_proxid, shuffle=False) :
	...
	proxid = numpy.ravel(test_set_proxid[:prox_score.shape[0]])


	if shuffle == True :
		shuffle_index = numpy.arange(len(y))
		numpy.random.shuffle(shuffle_index)
		prox_y_hat = prox_y_hat[shuffle_index]
		prox_score = prox_score[shuffle_index]
		dist_y_hat = dist_y_hat[shuffle_index]
		dist_score = dist_score[shuffle_index]
		y = y[shuffle_index]
		d = d[shuffle_index]
		c = c[shuffle_index]
		s = s[shuffle_index]
		proxid = proxid[shuffle_index]
	...
	proxid = proxid[logodds_isinf == False]
	...
	X = numpy.zeros((logodds.shape[0], 3))
	p = numpy.zeros((logodds.shape[0], 1))
	s_weights = numpy.zeros(logodds.shape[0])
	...
	#test_set_size = 0.75#0.90#0.75
	for test_set_size in [0.75] : #[0.75, 0.90] :
		for highlight_examples in ['no_highlight'] : #['no_highlight', 'highlight'] :
			...
			s_test = s[X_train.shape[0]:]
			proxid_test = proxid[X_train.shape[0]:]
			
			weights_train = s_weights[:int(X.shape[0] * test_set_size)]


			#Decrease training set
			'''X_train = X_train[-400:, :]
			p_train = p_train[-400:, :]
			weights_train = weights_train[-400:]'''
			...
			lr.fit(X_train, p_train)#, sample_weight=weights_train)

			print('Linear weights learned:')
			w = numpy.ravel(lr.coef_)
			print('Prox score weight = ' + str(w[0]))
			print('Dist score weight = ' + str(w[1]))
			print('Log distance weight = ' + str(w[2]))


			sort_index_d = numpy.argsort(d)
			f = plt.figure()
			plt.plot(d[sort_index_d], numpy.power(d[sort_index_d], w[2]), c='darkcyan', linewidth=3, linestyle='--')
			#plt.scatter(d[sort_index_d], numpy.power(d[sort_index_d], w[2]))

			max_d_w = numpy.max(numpy.power(d[sort_index_d], w[2]))

			plt.axis([0, numpy.max(d[sort_index_d]), 0, max_d_w])
			plt.xlabel('Site Distance')
			plt.ylabel(r'Expected odds increase')
			plt.title('Distance vs. Expected odds increase (w = ' + str(round(w[2], 2)) + ')', fontsize=14)

			plt.savefig("onesided2_dropout_cnn_apadb_distance_" + dataset + "_rc_" + str(count_filter) + '_test_size_' + str(test_set_size) + ".png")
			plt.savefig("onesided2_dropout_cnn_apadb_distance_" + dataset + "_rc_" + str(count_filter) + '_test_size_' + str(test_set_size) + ".svg")
			#plt.show()
			plt.close()

			f = plt.figure()
			plt.plot(d[sort_index_d], numpy.power(d[sort_index_d], w[2]) / numpy.power(d[sort_index_d][0], w[2]), c='darkcyan', linewidth=3, linestyle='--')
			#plt.scatter(d[sort_index_d], numpy.power(d[sort_index_d], w[2]))

			max_d_w = numpy.max(numpy.power(d[sort_index_d], w[2]) / numpy.power(d[sort_index_d][0], w[2]))

			plt.axis([0, numpy.max(d[sort_index_d]), 0, max_d_w])
			plt.xlabel('Site Distance')
			plt.ylabel(r'Expected odds increase')
			plt.title('Distance vs. odds ratio increase from min distance (w = ' + str(round(w[2], 2)) + ')', fontsize=14)

			plt.savefig("onesided2_dropout_cnn_apadb_distance_minratio_" + dataset + "_rc_" + str(count_filter) + '_test_size_' + str(test_set_size) + ".png")
			plt.savefig("onesided2_dropout_cnn_apadb_distance_minratio_" + dataset + "_rc_" + str(count_filter) + '_test_size_' + str(test_set_size) + ".svg")
			#plt.show()
			plt.close()
			...
			ax1.scatter(p_test_hat, p_test, c = 'black', s = 1 * numpy.pi * (2 * numpy.ones(1))**3, alpha=0.55)
			ax1.plot([-logodds_max,logodds_max], [-logodds_max,logodds_max], color='black')
			ax1.set_xlim([numpy.amin(p_test_hat) - 0.5,numpy.amax(p_test_hat) + 0.5])
			ax1.set_ylim([numpy.amin(p_test) - 0.5,numpy.amax(p_test) + 0.5])
			ax1.set_xlabel('Predicted Proximal Logodds', fontsize=18)
			ax1.set_ylabel('Observed Proximal Logodds', fontsize=18)
			ax1.set_title('APADB Testset (R^2 = ' + str(round(lr.score(X_test, p_test), 2)) + ', Acc = ' + str(round(accuracy, 2)) + ')' + ', ' + str(X_test.shape[0]) + ' UTRs', fontsize=14)
			...
			if highlight_examples == 'highlight' :
				for i in range(0, len(proxid_test)):

					gene = proxid_test[i].split('.')[0]
					sitenum = s[i] - int(proxid_test[i].split('.')[1])

					proxid_curr = gene + '.' + str(int(sitenum))

					annotation = '(' + proxid_curr + ')'

					if (p_test_hat[i] > 4 or p_test[i] > 4) and n_chosen_pos < 5 and sitenum in [0, 1, 2, 3] :

						n_chosen_pos += 1
						print(annotation)
						
						ax1.annotate(annotation, (p_test_hat[i], p_test[i]), size=18)
						ax1.scatter([p_test_hat[i]], [p_test[i]], c = 'orange', s = 1 * numpy.pi * (2 * numpy.ones(1))**3, alpha=0.85)
					if (p_test_hat[i] < -4 or p_test[i] < -4) and n_chosen_neg < 5 and sitenum in [0, 1, 2, 3] :
			...
			plt.savefig("onesided2_dropout_cnn_apadb_testset_" + dataset + "_rc_" + str(count_filter) + '_test_size_' + str(test_set_size) + '_' + highlight_examples + ".png")
			plt.savefig("onesided2_dropout_cnn_apadb_testset_" + dataset + "_rc_" + str(count_filter) + '_test_size_' + str(test_set_size) + '_' + highlight_examples + ".svg")
			#plt.show()
			...
			#r_packet_native_lr = numpy.load("native_lr_apadb_testset_roc_bio_larger_unfiltered.npy")
			r_packet_native_lr = numpy.load("native_lr_apadb_testset_roc_bio_larger.npy")
			r_hat_native_lr = numpy.ravel(r_packet_native_lr[:, 0])
			r_native_lr = numpy.ravel(r_packet_native_lr[:, 1])
			
			#r_packet_native_cnn = numpy.load("native_cnn_apadb_testset_roc_bio_larger_unfiltered.npy")
			r_packet_native_cnn = numpy.load("native_cnn_apadb_testset_roc_bio_larger.npy")
			r_hat_native_cnn = numpy.ravel(r_packet_native_cnn[:, 0])
			r_native_cnn = numpy.ravel(r_packet_native_cnn[:, 1])

			r_label_native_lr = numpy.zeros(len(r_native_lr))
			for i in range(0, len(r_native_lr)) :
				if r_native_lr[i] >= 0.5 :
					r_label_native_lr[i] = 1
				else :
					r_label_native_lr[i] = 0
			fpr_native_lr, tpr_native_lr, thresholds_native_lr = roc_curve(r_label_native_lr, r_hat_native_lr, pos_label=1)
			auc_native_lr = roc_auc_score(r_label_native_lr, r_hat_native_lr)

			r_label_native_cnn = numpy.zeros(len(r_native_cnn))
			for i in range(0, len(r_native_cnn)) :
				if r_native_cnn[i] >= 0.5 :
					r_label_native_cnn[i] = 1
				else :
					r_label_native_cnn[i] = 0
			fpr_native_cnn, tpr_native_cnn, thresholds_native_cnn = roc_curve(r_label_native_cnn, r_hat_native_cnn, pos_label=1)
			auc_native_cnn = roc_auc_score(r_label_native_cnn, r_hat_native_cnn)
			...
			l1, = plt.plot(fpr, tpr, c = 'purple', linewidth=4, alpha=0.7)
			#l2, = plt.plot(fpr_f, tpr_f, c = 'purple', linewidth=4, alpha=0.7)
			
			l3, = plt.plot(fpr_native_lr, tpr_native_lr, c = 'red', linewidth=4, alpha=0.7)
			l4, = plt.plot(fpr_native_cnn, tpr_native_cnn, c = 'brown', linewidth=4, alpha=0.7)
			...
			plt.legend((l1, l3, l4),
				('Synth. CNN (AUC = ' + str(round(auc, 2)) + ', R^2 = ' + str(round(lr.score(X_test, p_test), 2)) + ')',
				 #'Conf > 25% (AUC = ' + str(round(auc_f, 2)) + ', R^2 = ' + str(round(lr.score(X_test, p_test), 2)) + ')',
				 #'LR APADB   (AUC = ' + str(round(auc_native_lr, 2)) + ', R^2 = 0.16)',
				 #'CNN APADB  (AUC = ' + str(round(auc_native_cnn, 2)) + ', R^2 = 0.24)')
				 'LR APADB   (AUC = ' + str(round(auc_native_lr, 2)) + ', R^2 = 0.21)',
				 'CNN APADB  (AUC = ' + str(round(auc_native_cnn, 2)) + ', R^2 = 0.28)')
				, loc='lower right', shadow=False, fontsize=14)


			plt.title('APADB Test ROC' + ', ' + str(X_test.shape[0]) + ' UTRs', fontsize=14)

			plt.savefig("combined_onesided2_dropout_cnn_apadb_testset_roc_" + dataset + "_rc_" + str(count_filter) + '_test_size_' + str(test_set_size) + ".png")
			plt.savefig("combined_onesided2_dropout_cnn_apadb_testset_roc_" + dataset + "_rc_" + str(count_filter) + '_test_size_' + str(test_set_size) + ".svg")
			#plt.show()
			...
			ax1.scatter(p_hat, p, c = 'black', s = 1 * numpy.pi * (2 * numpy.ones(1))**3, alpha=0.55)
			ax1.plot([-logodds_max,logodds_max], [-logodds_max,logodds_max], color='black')
			ax1.set_xlim([numpy.amin(p_hat) - 0.5,numpy.amax(p_hat) + 0.5])
			ax1.set_ylim([numpy.amin(p) - 0.5,numpy.amax(p) + 0.5])
			ax1.set_xlabel('Predicted Proximal Logodds', fontsize=18)
			ax1.set_ylabel('Observed Proximal Logodds', fontsize=18)
			ax1.set_title('APADB LOOCV (R^2 = ' + str(round(test_rsquare, 2)) + ', Acc = ' + str(round(accuracy, 2)) + ')' + ', ' + str(X.shape[0]) + ' UTRs', fontsize=14)
			...
			if highlight_examples == 'highlight' :
				for i in range(0, len(proxid)):

					gene = proxid[i].split('.')[0]
					sitenum = s[i] - int(proxid[i].split('.')[1])

					proxid_curr = gene + '.' + str(int(sitenum))

					annotation = '(' + proxid_curr + ')'

					if (p_hat[i] > 4 or p[i] > 4) and n_chosen_pos < 5 and sitenum in [0, 1, 2, 3] :

						n_chosen_pos += 1
						print(annotation)
						
						ax1.annotate(annotation, (p_hat[i], p[i]), size=18)
						ax1.scatter([p_hat[i]], [p[i]], c = 'orange', s = 1 * numpy.pi * (2 * numpy.ones(1))**3, alpha=0.85)
					if (p_hat[i] < -4 or p[i] < -4) and n_chosen_neg < 5 and sitenum in [0, 1, 2, 3] :
			...
			plt.savefig("onesided2_dropout_cnn_apadb_loocv_" + dataset + "_rc_" + str(count_filter) + '_test_size_' + str(test_set_size) + '_' + highlight_examples + ".png")
			plt.savefig("onesided2_dropout_cnn_apadb_loocv_" + dataset + "_rc_" + str(count_filter) + '_test_size_' + str(test_set_size) + '_' + highlight_examples + ".svg")
			#plt.show()
			...
			#r_packet_native_lr = numpy.load("native_lr_apadb_loocv_roc_bio_larger_unfiltered.npy")
			r_packet_native_lr = numpy.load("native_lr_apadb_loocv_roc_bio_larger.npy")
			r_hat_native_lr = numpy.ravel(r_packet_native_lr[:, 0])
			r_native_lr = numpy.ravel(r_packet_native_lr[:, 1])
			...
			plt.legend((l1, l3, l4),
				('Synth. CNN (AUC = ' + str(round(auc, 2)) + ', R^2 = ' + str(round(test_rsquare, 2)) + ')',
				 #'Conf > 25% (AUC = ' + str(round(auc_f, 2)) + ', R^2 = ' + str(round(test_rsquare, 2)) + ')',
				 #'LR APADB   (AUC = ' + str(round(auc_native_lr, 2)) + ', R^2 = 0.16)',
				 #'CNN APADB  (AUC = ' + str(round(auc_native_cnn, 2)) + ', R^2 = 0.24)')
				 'LR APADB   (AUC = ' + str(round(auc_native_lr, 2)) + ', R^2 = 0.21)',
				 'CNN APADB  (AUC = ' + str(round(auc_native_cnn, 2)) + ', R^2 = 0.28)')
			...
			plt.title('APADB LOOCV ROC' + ', ' + str(X.shape[0]) + ' UTRs', fontsize=14)

			#plt.subplots_adjust(top=0.85)

			plt.savefig("combined_onesided2_dropout_cnn_apadb_loocv_" + dataset + "_rc_" + str(count_filter) + '_test_size_' + str(test_set_size) + ".png")
			plt.savefig("combined_onesided2_dropout_cnn_apadb_loocv_" + dataset + "_rc_" + str(count_filter) + '_test_size_' + str(test_set_size) + ".svg")
			#plt.show()
...
if __name__ == '__main__':
	evaluate_cnn()#'bio_larger_unfiltered'
