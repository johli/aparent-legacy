...
class LogisticRegression(object):
	...
	def __init__(self, input, L_input, n_in, n_out, load_model = False, w_file = '', b_file = '', store_as_w_file = None, store_as_b_file = None):
		...
		standard_range = numpy.arange(50, 90).tolist()
		standard_range_versus = numpy.concatenate([numpy.arange(0, 50), numpy.arange(90, 186)]).tolist()
	...
	def recall(self):
		...
		#standard_range = numpy.arange(60, 100).tolist()
		#standard_range_versus = numpy.concatenate([numpy.arange(20, 60), numpy.arange(100, 186)]).tolist()
...
def load_output_data(dataset, splice_site_indexes=[1], shuffle_index=None, balance_data=False, balance_test_set=False, count_filter=None, misprime_index=None, balance_libraries=False, balance_sublibraries=False, balance_all_libraries=False):#,1,2
	...
	y = numpy.matrix(numpy.load('../apa_analysis_bio/apa_human/apa_' + dataset + '_output.npy'))#NOT .T
	...
	c = numpy.ravel(numpy.load('../apa_analysis_bio/apa_human/apa_' + dataset + '_count.npy'))
...
def load_input_data(dataset, prefix='prox', shuffle=False, balance_data=False, balance_test_set=False, count_filter=None, misprime_filter=False, balance_libraries=False, balance_sublibraries=False, balance_all_libraries=False):
	...
	if use_cache == False :
		...
		loader = numpy.load('../apa_analysis_bio/apa_human/npz_apa_seq_' + dataset + '_' + prefix + 'input.npz')
		...
		y = numpy.ravel(numpy.load('../apa_analysis_bio/apa_human/apa_' + dataset + '_output.npy')[:,0])
		c = numpy.ravel(numpy.load('../apa_analysis_bio/apa_human/apa_' + dataset + '_count.npy'))
		L = numpy.zeros(len(c))
		#L[:] = 11

		d = numpy.load('../apa_analysis_bio/apa_human/apa_' + dataset + '_distance.npy')
		c_input = numpy.load('../apa_analysis_bio/apa_human/apa_' + dataset + '_count.npy')
		s_input = numpy.load('../apa_analysis_bio/apa_human/apa_' + dataset + '_sites.npy')
		...
		train_set_s = theano.shared(numpy.asarray(s_train, dtype=theano.config.floatX), borrow=True)
		valid_set_s = theano.shared(numpy.asarray(s_validation, dtype=theano.config.floatX), borrow=True)
		test_set_s = theano.shared(numpy.asarray(s_test, dtype=theano.config.floatX), borrow=True)
		...
		rval = [train_set_x, valid_set_x, test_set_x, shuffle_index, train_set_L, valid_set_L, test_set_L, None, train_set_d, valid_set_d, test_set_d, train_set_c, valid_set_c, test_set_c, train_set_s, valid_set_s, test_set_s]
