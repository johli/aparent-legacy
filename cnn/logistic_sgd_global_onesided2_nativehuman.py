...
class LinearRegression(object):
	...
	def negative_log_likelihood(self, y):
		...
		#return T.mean(T.multiply(y - self.p_y_given_x, y - self.p_y_given_x))
		#return T.dot((y - self.p_y_given_x).reshape((y.shape[0])), (y - self.p_y_given_x).reshape((y.shape[0]))) #/ y.shape[0].astype(theano.config.floatX)
		
		return T.dot(y - self.p_y_given_x, y - self.p_y_given_x) / y.shape[0].astype(theano.config.floatX)
		#return T.dot(y - self.p_y_given_x, y - self.p_y_given_x) / y.shape[0].astype(theano.config.floatX)
	...
	def rsquare(self, y):
		return 1.0 - (T.dot(y - self.p_y_given_x, y - self.p_y_given_x) / T.dot(y - T.mean(y), y - T.mean(y)))
	
	def sse(self, y):
		return T.dot(y - self.p_y_given_x, y - self.p_y_given_x)
	
	def sst(self, y):
		return T.dot(y - T.mean(y), y - T.mean(y))
...
def load_output_data(dataset, splice_site_indexes=[1], shuffle_index=None, balance_data=False, balance_test_set=False, count_filter=None, misprime_index=None, balance_libraries=False, balance_sublibraries=False, balance_all_libraries=False):#,1,2
	...
	train_set_split = 0.74
	valid_set_split = 0.25
	...
	'''laplace = 10.0
	prox_c = numpy.ravel(y[:, 1]) * c + laplace
	dist_c = numpy.ravel(y[:, 0]) * c + laplace

	y[:, 1] = numpy.reshape(prox_c / (prox_c + dist_c), (len(prox_c), 1))
	y[:, 0] = 1.0 - y[:, 1]'''
...
def load_input_data(dataset, prefix='prox', shuffle=False, shuffle_index_in=None, balance_data=False, balance_test_set=False, count_filter=None, misprime_filter=False, balance_libraries=False, balance_sublibraries=False, balance_all_libraries=False):
	#############
	# LOAD DATA #
	#############
	...
	if use_cache == False :
		...
		shuffle_index = None
		if shuffle == True :
			if shuffle_index_in is None :
				shuffle_index = numpy.arange(X.shape[0])
				numpy.random.shuffle(shuffle_index)
			else :
				shuffle_index = shuffle_index_in
			X = X[shuffle_index,:]
			L = L[shuffle_index]
			c = c[shuffle_index]
			y = y[shuffle_index]
			d = d[shuffle_index]
			c_input = c_input[shuffle_index]
			s_input = s_input[shuffle_index]
		...
		if count_filter is not None :
			...
			y = y[c > count_filter]
			d = d[c > count_filter]
			c_input = c_input[c > count_filter]
			s_input = s_input[c > count_filter]
			c = c[c > count_filter]
