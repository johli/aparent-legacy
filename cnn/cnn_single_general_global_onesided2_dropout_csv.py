...
from logistic_sgd_global_onesided2 import LogisticRegression
...
def evaluate_cnn():#_pasaligned

	batch_size = 1

	train_set_x = theano.shared(value=sp.csr_matrix(numpy.zeros((1, 185)), dtype=numpy.int8), borrow=True)
	train_set_y = theano.shared(value=numpy.zeros((1, 2), dtype=theano.config.floatX), borrow=True)
	train_set_L = theano.shared(value=numpy.zeros((1, 36), dtype=theano.config.floatX), borrow=True)
	train_set_d = theano.shared(value=numpy.zeros((1, 1), dtype=theano.config.floatX), borrow=True)

	valid_set_x = theano.shared(value=sp.csr_matrix(numpy.zeros((1, 185)), dtype=numpy.int8), borrow=True)
	valid_set_y = theano.shared(value=numpy.zeros((1, 2), dtype=theano.config.floatX), borrow=True)
	valid_set_L = theano.shared(value=numpy.zeros((1, 36), dtype=theano.config.floatX), borrow=True)
	valid_set_d = theano.shared(value=numpy.zeros((1, 1), dtype=theano.config.floatX), borrow=True)
	...
	cnn.set_data(train_set_x, train_set_y, train_set_L, train_set_d)
...
def test_csv(cnn) :
	...
	data = pd.read_csv('array_unfiltered_grouped_seq_count_10_dropnans_False.csv',sep=',')
	print(len(data))

	seqs = list(data.seq.str.slice(21, 206).values)
	libs = list(data.gene.values)
	...
	for i in range(0, len(seqs)) :
		...
		x = numpy.zeros((1, 185, 4), dtype=theano.config.floatX)
		...
		L_fake = numpy.zeros((1, 36), dtype=theano.config.floatX)
		d_fake = numpy.ones((1, 1), dtype=theano.config.floatX)
		...
		y_hat = cnn.get_online_prediction(x, L_fake, d_fake)[0]
		log_hat = numpy.log(y_hat / (1.0 - y_hat))

		y_hats.append(round(y_hat, 5))
		log_hats.append(round(log_hat, 3))

	data['predicted_usage'] = y_hats
	data['predicted_logodds'] = log_hats

	data = data[['seq', 'master_seq', 'experiment', 'subexperiment', 'predicted_usage', 'predicted_logodds']]

	data.to_csv('array_unfiltered_grouped_seq_count_10_dropnans_False_predicted_isoform_dropout_pasaligned.csv', header=True, index=False, sep=',')
