...
def load_output_data(dataset):#,1,2
	...
	'''snptype_prune_index = (snptype == 1) | (snptype == 2)
	y_ref = y_ref[snptype_prune_index,:]
	y_var = y_var[snptype_prune_index,:]
	snpregion = snpregion[snptype_prune_index]
	snptype = snptype[snptype_prune_index]

	snpregion_prune_index = (snpregion == 1) | ((snpregion == 3) | (snpregion == 2))
...
def load_input_data(dataset):
	...
	X = spio.loadmat('snps/apa_sym_prx_fullseq_global_' + dataset + '_input.mat')
