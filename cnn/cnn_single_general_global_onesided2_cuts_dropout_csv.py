...
def evaluate_cnn(dataset='general_cuts_antimisprime_orig'):#_pasaligned
	...
	train_set_y = theano.shared(value=sp.csr_matrix(numpy.zeros((1, 186)), dtype=theano.config.floatX), borrow=True)
	...
	valid_set_y = theano.shared(value=sp.csr_matrix(numpy.zeros((1, 186)), dtype=theano.config.floatX), borrow=True)
...
def test_csv(cnn) :
	...
	for i in range(0, len(seqs)) :
		...
		cuts_hats.append(cuts_hat.reshape(1, -1))

	cuts_hats = sp.csr_matrix(numpy.vstack(cuts_hats))
	
	spio.savemat('array_unfiltered_grouped_seq_count_10_dropnans_False_predicted_cuts_dropout', {'cuts' : cuts_hats})
