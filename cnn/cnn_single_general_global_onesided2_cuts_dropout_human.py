...
from logistic_sgd_global_onesided2_cuts_human import LogisticRegression#, load_input_data, load_output_data
from logistic_sgd_global_onesided2_apadb import load_input_data, load_output_data#, LogisticRegression
...
def evaluate_cnn(dataset='apadb_v2'):#'bio_large'):
	...
	train_set_y_spoof = theano.shared(sp.csr_matrix(numpy.zeros((train_set_L.eval().shape[0], 186))), borrow=True)
	valid_set_y_spoof = theano.shared(sp.csr_matrix(numpy.zeros((valid_set_L.eval().shape[0], 186))), borrow=True)
	test_set_y_spoof = theano.shared(sp.csr_matrix(numpy.zeros((test_set_L.eval().shape[0], 186))), borrow=True)
	...
	run_name = '_Global2_Onesided2Cuts2AntimisprimeOrigDropout_HumanAPADB'
	...
	cnn = DualCNN(
		(train_set_x_prox, train_set_y_spoof, train_set_L, train_set_d_prox),
		(valid_set_x_prox, valid_set_y_spoof, valid_set_L, valid_set_d_prox),
		learning_rate=0.1,
		drop=0.2,
		n_epochs=10,
		nkerns=[70, 110, 70],
	...
	fit_apadb(cnn, test_set_x_prox, test_set_x_dist, test_set_d_prox, test_set_d_dist, test_set_y, test_set_L, test_set_d, test_set_c, test_set_s, test_set_y_spoof, dataset, count_filter)

def fit_apadb(cnn, test_set_x_prox, test_set_x_dist, test_set_d_prox, test_set_d_dist, test_set_y, test_set_L, test_set_d, test_set_c, test_set_s, test_set_y_spoof, dataset, count_filter) :
	cnn.set_data(test_set_x_prox, test_set_y_spoof, test_set_L, test_set_d_prox)


	prox_y_hat = numpy.ravel(cnn.get_prediction())
	prox_score = numpy.ravel(numpy.sum(cnn.get_class_score()[:, 60:100], axis=1))

	cnn.set_data(test_set_x_dist, test_set_y_spoof, test_set_L, test_set_d_dist)
	dist_y_hat = numpy.ravel(cnn.get_prediction())
	dist_score = numpy.ravel(numpy.sum(cnn.get_class_score()[:, 60:100], axis=1))
	...
	for i in range(0, logodds.shape[0]) :
		X[i, 0] = prox_logodds_hat[i]#prox_score[i]
		X[i, 1] = dist_logodds_hat[i]#dist_score[i]
		X[i, 2] = log_d[i]

		#X[i, 4] = prox_logodds_hat[i] * log_d[i]
		#X[i, 3] = dist_score[i] / (log_d[i])
		#X[i, 3] = dist_logodds_hat[i] / (log_d[i])

		#X[i, 4] = prox_score[i] * log_d[i]

		#X[i, 4] = log_c[i]
		#X[i, 5] = prox_logodds_hat[i] * dist_logodds_hat[i]
	...
	plt.scatter(p_test_hat, p_test, c = 'black', s = 1 * numpy.pi * (2 * numpy.ones(1))**4, alpha=0.55)
	plt.plot([-logodds_max,logodds_max], [-logodds_max,logodds_max], color='black')
	plt.xlim([numpy.amin(p_test_hat) - 0.5,numpy.amax(p_test_hat) + 0.5])
	plt.ylim([numpy.amin(p_test) - 0.5,numpy.amax(p_test) + 0.5])
	plt.xlabel('Predicted Proximal Logodds', fontsize=18)
	plt.ylabel('Observed Proximal Logodds', fontsize=18)
	plt.title('APADB Testset (R^2 = ' + str(round(lr.score(X_test, p_test), 2)) + ', Acc = ' + str(round(accuracy, 2)) + ')' + ', ' + str(X_test.shape[0]) + ' UTRs', fontsize=14)

	#plt.subplots_adjust(top=0.85)

	plt.savefig("onesided2_cuts_dropout_cnn_apadb_testset_" + dataset + "_rc_" + str(count_filter) + ".png")
	plt.savefig("onesided2_cuts_dropout_cnn_apadb_testset_" + dataset + "_rc_" + str(count_filter) + ".svg")
	plt.show()
	...
	plt.savefig("combined_onesided2_cuts_dropout_cnn_apadb_testset_roc_" + dataset + "_rc_" + str(count_filter) + ".png")
	plt.savefig("combined_onesided2_cuts_dropout_cnn_apadb_testset_roc_" + dataset + "_rc_" + str(count_filter) + ".svg")
	plt.show()
	...
	plt.scatter(p_hat, p, c = 'black', s = 1 * numpy.pi * (2 * numpy.ones(1))**4, alpha=0.55)
	plt.plot([-logodds_max,logodds_max], [-logodds_max,logodds_max], color='black')
	plt.xlim([numpy.amin(p_hat) - 0.5,numpy.amax(p_hat) + 0.5])
	plt.ylim([numpy.amin(p) - 0.5,numpy.amax(p) + 0.5])
	plt.xlabel('Predicted Proximal Logodds', fontsize=18)
	plt.ylabel('Observed Proximal Logodds', fontsize=18)
	plt.title('APADB LOOCV (R^2 = ' + str(round(test_rsquare, 2)) + ', Acc = ' + str(round(accuracy, 2)) + ')' + ', ' + str(X.shape[0]) + ' UTRs', fontsize=14)

	#plt.subplots_adjust(top=0.85)

	plt.savefig("onesided2_cuts_dropout_cnn_apadb_loocv_" + dataset + "_rc_" + str(count_filter) + ".png")
	plt.savefig("onesided2_cuts_dropout_cnn_apadb_loocv_" + dataset + "_rc_" + str(count_filter) + ".svg")
	plt.show()
	...
	plt.savefig("combined_onesided2_cuts_dropout_cnn_apadb_loocv_" + dataset + "_rc_" + str(count_filter) + ".png")
	plt.savefig("combined_onesided2_cuts_dropout_cnn_apadb_loocv_" + dataset + "_rc_" + str(count_filter) + ".svg")
	plt.show()
